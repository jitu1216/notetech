...
class MaintenanceController extends Controller
{
    ...
    public function studentmaintenance($id)
    {
        ...
        return view('school.schoolmaintenance.student-maintenance', compact(
            'finalarray',
            'schoolclass',
            'mark',
            'studentList',
    ...
    public function studentMaintenanceRecord($id = null)
    {
        ...
        $class = $id;
        if ($class != null) {
            $maintenance = Student::where(['academic_session' => $academic, 'school_id' => $school->id, 'status' => '2', 'class_id' => $class])->with('maintenances', 'schoolClass')->get();
        } else {
            $maintenance = Student::where(['academic_session' => $academic, 'school_id' => $school->id, 'status' => '2'])->with('maintenances', 'schoolClass')->get();
        }

        $item = Item::where(['school_id' => $school->id, 'academic_session' => $academic])->get();
        $sortedAttendance = $maintenance->sortBy(function ($maintenance) use ($sortedArray) {
            return array_search($maintenance->schoolClass->classname, $sortedArray);
        });
        $maintenance = $sortedAttendance->values();
